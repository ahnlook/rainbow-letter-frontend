...
function ForeLetterDetailForm({
  ...
  const { user, pet, letter, letters } = letterData;
  ...
  const handleRegenerateClick = () => {
    if (letter?.submitTime) return alert('이미 답장을 보낸 편지입니다.');
  ...
  useEffect(() => {
    if (letter && (letter?.promptA || letter?.promptB)) {
      setNewContentValue({
        promptA: letter?.promptA,
        promptB: letter?.promptB,
      });
    }
  }, [letter?.promptA, letter?.promptB]);

  useEffect(() => {
    setSelectedPrompt(letter?.promptA ? 'promptA' : 'promptB');
  }, [letter?.promptA, letter?.promptB]);
  ...
  const replyStatus = getReplyStatus(letter.petInitiatedLetterStatus);
          ...
          <div className="flex justify-between p-2 text-gray-500">
            <div className="flex gap-x-10">
              ...
              <div className="flex flex-col gap-y-1">
                ...
                <span className="whitespace-nowrap">{user.count || 0}</span>
                <span className="whitespace-nowrap">
                  {formatDateToYYMMDD(user.lastLoggedIn)}
                </span>
              </div>
            </div>
          </div>
        ...
        <section className="flex h-full flex-[2] flex-col gap-y-2">
          <header className="flex w-full justify-between">
            ...
            <div className="flex flex-col">
              {!!letter?.submitTime && (
                <span>발송일 {formatDateToYYMMDDHHSS(letter?.submitTime)}</span>
              )}
            </div>
          </header>
...
export default ForeLetterDetailForm;
